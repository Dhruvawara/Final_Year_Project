...
class amazon_product_review_scraper:
    ...
    def scrape(self):
        ...
        title = soup.find_all("img",{"data-hook":"cr-product-image"})
        # print(title[0]['alt'],title[0]['src'],title[0]['data-a-hires'])

        # returning df
        return pd.DataFrame(self.reviews_dict),title[0]['alt'],title[0]['src']
